...
public class RecyclerAdapterContactWeb extends RecyclerView.Adapter<RecyclerAdapterContactWeb.ViewHolder> {

    private List<WebTeamModels> mWebTeams;
    ...
    public RecyclerAdapterContactWeb(Context context, List<WebTeamModels> dataset) {
        mWebTeams = dataset;
        mContext = context;
    }
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        final WebTeamModels webTeamModels = mWebTeams.get(position);

        Glide.with(mContext).load(webTeamModels.getImgurl()).into(viewHolder.mImg);
        viewHolder.mName.setText(webTeamModels.getName());
        viewHolder.mRank.setText(webTeamModels.getRank());
        viewHolder.setOnClickListener(new ItemClickListener() {
            @Override
            public void onClick(View view, int position, boolean isLongClick, MotionEvent motionEvent) {
                if (isLongClick) {
                    Toast.makeText(mContext, webTeamModels.getRank(), Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(mContext, webTeamModels.getName(), Toast.LENGTH_SHORT).show();
                }
